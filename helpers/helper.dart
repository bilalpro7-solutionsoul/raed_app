...
//        },
  ...
  static bool canDelivery(Market _market, {List<Cart> carts}) {
    ...
    if (_distance == 0 && !deliveryAddress.value.isUnknown()) {
      if(_market.latitude != null
          && deliveryAddress.value.longitude != null
      && _market.longitude != null
      && deliveryAddress.value.latitude != null){
        try{
          _distance = sqrt(pow(69.1 * (double.parse(_market.latitude ?? 0.0) - deliveryAddress.value.latitude ?? 0.0), 2) +
              pow(69.1 * (deliveryAddress.value.longitude ?? 0.0 - double.parse(_market.longitude ?? 0.0)) * cos(double.parse(_market.latitude) / 57.3), 2));
          _distance ??= 0.0;
        }catch(e){
          _distance = 0.0;
        }
